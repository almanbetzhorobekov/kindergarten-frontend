...
import { CreateGroupDTO, GroupDTO, UpdateGroupDTO } from "api/group.type";
...
export const groupAPI = {
  ...
  create: (data: CreateGroupDTO): Promise<GroupDTO> =>
    fetchService(GROUPS_URL, { method: "POST", body: data }),

  update: (uuid: string, data: UpdateGroupDTO) =>
    fetchService(`${GROUPS_URL}/${uuid}`, { method: "PUT", body: data }),

  delete: (uuid: string) =>
    fetchService(`${GROUPS_URL}/${uuid}`, { method: "DELETE" }),
};
